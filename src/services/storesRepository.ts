// Stores repository — same adapter pattern as distributorsRepository.ts,
// one level down the Bab 5 hierarchy (a Store optionally belongs to a
// Distributor). api/stores GET includes the `distributor` relation.
//
// Note (api/stores/[id].ts): PUT is restricted server-side to approver
// roles (Super Admin / Sales Manager / Master Data Admin), same as
// distributors.

import type { Store, ApprovalStatus, StoreDistributorRef } from '@/types/store';
import type { Result } from '@/types/result';

function getAuthToken(): string | undefined {
  try {
    const raw = localStorage.getItem('salesMonitorUser');
    if (!raw) return undefined;
    return JSON.parse(raw)?.accessToken;
  } catch {
    return undefined;
  }
}

async function apiFetch<T>(path: string, options: RequestInit = {}): Promise<Result<T>> {
  try {
    const token = getAuthToken();
    const res = await fetch(path, {
      ...options,
      headers: {
        'Content-Type': 'application/json',
        ...(token ? { Authorization: `Bearer ${token}` } : {}),
        ...options.headers,
      },
    });
    const body = res.status === 204 ? { success: true } : await res.json();
    if (!res.ok) {
      if (res.status === 401) {
        return { success: false, error: 'Sesi login tidak valid atau sudah berakhir. Silakan logout dan login kembali.' };
      }
      return { success: false, error: body.error ?? 'Terjadi kesalahan pada server' };
    }
    return body as Result<T>;
  } catch (error) {
    console.error(`storesRepository: request failed (${path}):`, error);
    return { success: false, error: 'Tidak dapat terhubung ke server. Periksa koneksi Anda dan coba lagi.' };
  }
}

const STATUS_OUT: Record<ApprovalStatus, string> = {
  pending: 'PENDING',
  approved: 'APPROVED',
  rejected: 'REJECTED',
};
const STATUS_IN: Record<string, ApprovalStatus> = {
  PENDING: 'pending',
  APPROVED: 'approved',
  REJECTED: 'rejected',
};

function toApiPayload(input: Partial<Store>): Record<string, unknown> {
  const { status, distributor, ...rest } = input as Record<string, unknown> & {
    status?: ApprovalStatus;
    distributor?: StoreDistributorRef | null;
  };
  const payload: Record<string, unknown> = { ...rest };
  if (status) payload.status = STATUS_OUT[status];
  return payload;
}

function fromApiStore(row: any): Store {
  return {
    id: row.id,
    code: row.code,
    name: row.name,
    distributorId: row.distributorId ?? null,
    distributor: row.distributor
      ? { id: row.distributor.id, code: row.distributor.code, name: row.distributor.name }
      : null,
    address: row.address ?? '',
    gpsLat: row.gpsLat ?? null,
    gpsLng: row.gpsLng ?? null,
    status: STATUS_IN[row.status] ?? 'pending',
    submittedById: row.submittedById ?? null,
    submittedAt: row.submittedAt ? new Date(row.submittedAt) : null,
    decidedById: row.decidedById ?? null,
    decidedAt: row.decidedAt ? new Date(row.decidedAt) : null,
    rejectionNote: row.rejectionNote ?? '',
    salesRepId: row.salesRepId ?? null,
    salesRep: row.salesRep
      ? { id: row.salesRep.id, name: row.salesRep.name, email: row.salesRep.email }
      : null,
    createdAt: new Date(row.createdAt),
    updatedAt: new Date(row.updatedAt),
  };
}

export const storesRepository = {
  async getAll(): Promise<Result<Store[]>> {
    const res = await apiFetch<any[]>('/api/stores');
    if (!res.success || !res.data) return res as Result<Store[]>;
    return { success: true, data: res.data.map(fromApiStore) };
  },

  async getById(id: string): Promise<Result<Store>> {
    const res = await apiFetch<any>(`/api/stores/${id}`);
    if (!res.success || !res.data) return res as Result<Store>;
    return { success: true, data: fromApiStore(res.data) };
  },

  async create(input: Partial<Store>): Promise<Result<Store>> {
    const res = await apiFetch<any>('/api/stores', {
      method: 'POST',
      body: JSON.stringify(toApiPayload(input)),
    });
    if (!res.success || !res.data) return res as Result<Store>;
    return { success: true, data: fromApiStore(res.data) };
  },

  async update(id: string, updates: Partial<Store>): Promise<Result<Store>> {
    const res = await apiFetch<any>(`/api/stores/${id}`, {
      method: 'PUT',
      body: JSON.stringify(toApiPayload(updates)),
    });
    if (!res.success || !res.data) return res as Result<Store>;
    return { success: true, data: fromApiStore(res.data) };
  },

  async decide(id: string, status: 'approved' | 'rejected', rejectionNote?: string): Promise<Result<Store>> {
    return this.update(id, { status, ...(rejectionNote ? { rejectionNote } : {}) });
  },

  async remove(id: string): Promise<Result<void>> {
    return apiFetch<void>(`/api/stores/${id}`, { method: 'DELETE' });
  },
};
